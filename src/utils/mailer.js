...
function buildMeetingScheduleHtml(name, date, time, meetLink) {
  return `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"></head>
<body style="margin:0;padding:0;background:#f5f5fa;font-family:'Segoe UI',system-ui,sans-serif">
  <div style="max-width:600px;margin:0 auto;background:#ffffff">
    ...
    <div style="padding:32px 28px 24px">
      <h1 style="margin:0 0 4px;font-size:22px;color:#1e1e30;text-align:center">Rotaract 3234 DO Screening</h1>
      <p style="margin:0 0 24px;font-size:13px;color:#8587b3;text-align:center">District Officials Recruitment — Interview Schedule</p>

      <p style="font-size:15px;color:#34344b;line-height:1.7">Dear <strong>${name}</strong>,</p>

      <p style="font-size:14px;color:#34344b;line-height:1.7">
        We are pleased to inform you that your screening interview has been scheduled on <strong style="color:#e71e6d">${date}</strong> by <strong style="color:#e71e6d">${time}</strong>.
      </p>

      <!-- Meeting Link -->
      <div style="margin:20px 0;background:#f0f4ff;border:1px solid #d0d8f0;border-radius:8px;padding:14px 16px;text-align:center">
        <div style="font-size:10px;text-transform:uppercase;letter-spacing:1.5px;color:#8587b3;margin-bottom:6px">Meeting Link</div>
        <a href="${meetLink}" style="font-size:15px;font-weight:700;color:#0f9fd8;text-decoration:none;word-break:break-all">${meetLink}</a>
      </div>
      ...
      <p style="font-size:14px;color:#34344b;line-height:1.7;margin-top:24px">
        Looking forward to see you. Kindly reach out to me for any queries.
      </p>

      <p style="font-size:14px;color:#34344b;line-height:1.7;margin-top:8px">
        Thanks and Regards,<br>
        <strong style="color:#1e1e30">Rtr. IPP. PHF. Venkatesh A.</strong><br>
        <span style="color:#8587b3;font-size:13px">District Rotaract Secretary 26-27</span>
      </p>
    </div>
...
async function sendMeetingSchedule({ email, name, date, time, meetLink }) {
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.warn('SMTP not configured — skipping email');
    return { success: false, error: 'SMTP not configured' };
  }
  ...
  try {
    await transporter.sendMail({
      from: SENDER,
      to: email,
      subject: `Screening Interview Scheduled — ${date} | Rotaract District 3234`,
      html: buildMeetingScheduleHtml(name, date, time, meetLink),
    });
    console.log(`Meeting schedule email sent to ${email}`);
    return { success: true };
  } catch (err) {
    console.error(`Failed to send meeting schedule to ${email}:`, err.message);
    return { success: false, error: err.message };
  }
}

module.exports = { sendAcknowledgement, sendAdminNotification, sendOTPEmail, sendInterviewNotification, sendMeetingSchedule };
